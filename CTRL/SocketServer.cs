    ...
    class SocketServer
    {
        ...
        public async void Start(string listenerPrefix)
        {
            ...
            catch (Exception e)
            {
                ...
                try
                {
                    System.Diagnostics.Process.Start("netsh.exe", "http delete urlacl url=http://+:80/wsDemo/");
                    //http://+:80/wsDemo/
                }
                catch (Exception e2)
                {
            ...
            try
            {
                ...
                while (webSocket.State == WebSocketState.Open)
                {
                    ...
                    while (true)
                    {
                        ...
                        if (Encoding.UTF8.GetString(buffer).Contains("__ping__"))
                        {
                            //send pong, reset
                            Console.WriteLine("Sending pong back");
                            string s = "__pong__";
                            //int bufferSize2 = 1000;
                            var buffer2 = Encoding.UTF8.GetBytes(s);
                            var offset2 = 0;
